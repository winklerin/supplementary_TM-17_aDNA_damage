...
# load age model
```{r}
age_model_RAW=read.csv('TM17_Stalagmite_Age_Model.csv', skip=1, col.names = c( 'cm_from_top', 'age_yrs_bf_1950', 'age_yrs_bf_1950_upper_CI', 'age_yrs_bf_1950_lower_CI')) %>% filter(if_any(everything(), ~ !is.na(.x)))
```
...
```{r}
intervals=read.csv('Rock_Cube_Sizes_from_imageJ.csv', sep=';')
```
...
```{r}
write.table(age_model_summarized, 'summarized_age_model.csv', quote = F, row.names = F)
```
